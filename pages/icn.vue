<template>
  <div class="flex justify-center items-center m-1">
    <input type="number" v-model="targetClubId" class="mr-2 w-24 h-12 rounded" placeholder="Club"
      v-on:keyup.enter="getData()" />

    <input type="number" v-model="targetRound" class="mr-2 w-24 h-12 rounded" placeholder="Round"
      v-on:keyup.enter="getData()" />

    <button @click="getData()" class="border border-gray-500 p-3 h-12 mr-2 rounded"
      :disabled="!targetClubId || !targetRound" :class="{ 'opacity-20': !targetClubId || !targetRound }">
      GO
    </button>
  ...
  <div id="targetNode" style="
    ...
    <div v-for="division in divisions">
      ...
      <div style="
          ...
          margin: 10px;
        ">
        <div v-for="(result, i) in division.results" style="display: flex"
          :style="i % 2 ? 'background-color: rgb(241 245 249);' : ''">
          <div style="width: 256px; padding: 4px; text-align: right">
            {{ result.fullname_home }}
            <small style="color: rgb(107 114 128)">({{ result.rating_home }})</small>
          </div>
          ...
          <div style="width: 256px; padding: 4px">
            ...
            <small style="color: rgb(107 114 128)">({{ result.rating_visit }})</small>
          </div>
        </div>
  ...
  for (const division of reqDivisions) {
    ...
    const results = await _fetch(
      'https://proxy.echiquiermosan.be/https://www.frbe-kbsb-ksb.be/api/v1/interclubs/anon/icresultdetails',
      {
        ...
        pairingnr_home: targetEncounter.pairingnr_home,
        pairingnr_visit: targetEncounter.pairingnr_visit,
      }
    )
